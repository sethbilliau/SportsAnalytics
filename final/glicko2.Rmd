...
get_log_likelihood = function(newdata, ratings){
  ...
  ratings = ratings %>% 
    mutate(Rating = (Rating-1500)/173.7178,
           Deviation = Deviation/173.7178)
...
m = eval_models(metric = 'log-likelihood', 
            df = full_df, 
            newdata = clp_df, 
            min_games_list = append(list(1), seq(5, 50, 5)), 
            rating_period_list = list(1,2,3,4))
...
```{r}
# finding the parameters of the optimal model
...
min_games = 1 # change this accordingly
...
full_df_pooled %>% 
  ...
  count() %>% 
  group_by(year) %>% 
  summarize(avg_matches_per_player = round(mean(n),2))
```

```{r}
# Finding rankings for specific players
clp_players = unique(c(as.list(clp_df$playerA), as.list(clp_df$playerB)))
top_players = list('Ko Ping-Chung', 'Joshua Filler', 'Shane Van Boening', 'Fedor Gorst', 'Niels Feijen',
                   'Chang Jung-Lin', 'Ko Pin Yi', 'Alex Pagulayan', 'Jayson Shaw', 'Alex Kazakis')

clp_rankings = get_ratings(df = full_df, min_games = 1, rating_period = 4, tau_val = 0.3) %>% 
  filter(Player %in% clp_players) %>% 
  select(Player) %>% 
  mutate(ranking = row_number())

top_rankings = get_ratings(df = full_df, min_games = 1, rating_period = 4, tau_val = 0.3) %>% 
